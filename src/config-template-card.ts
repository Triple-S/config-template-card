...
export class ConfigTemplateCard extends LitElement {
  ...
  public setConfig(config?: Config): void {
    ...
    this._globalConfig = this.getLovelacePanelConfig();
    ...
    if (ha?.shadowRoot) {
      ...
      if (haMain?.shadowRoot) {
        ...
        if (haPanel?.shadowRoot) {
          const haRoot : any = haPanel.shadowRoot.querySelector('hui-root');
          if (haRoot) {
            const ll = haRoot.lovelace;
            ll.current_view = haRoot.___curView;
            return ll;
          }
        }
      }
  ...
  private getLovelacePanelConfig() : any {
    ...
    return {
      svars: lovelace?.config?.config_template_card_staticVars,
      vars: lovelace?.config?.config_template_card_vars,
    };
  }
  ...
  private getLovelaceViewConfig() : any {
    ...
    return {
      svars: lovelace?.config?.views[lovelace.current_view]?.config_template_card_staticVars,
      vars: lovelace?.config?.views[lovelace.current_view]?.config_template_card_vars,
    };
  }
  ...
  protected shouldUpdate(changedProps: PropertyValues): boolean {
    ...
    if (oldHass) {
      ...
      const localEntities = this._evaluateStructureSimple(varMgr, this._config.entities, false);
      if (Array.isArray(localEntities)) {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        entities.push(...localEntities);
      }
      const panelEntities = this._evaluateStructureSimple(varMgr, this.getLovelacePanelEntities(), false);
      if (Array.isArray(panelEntities)) {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
        entities.push(...panelEntities);
      }
      const viewEntities = this._evaluateStructureSimple(varMgr, this.getLovelaceViewEntities(), false);
